Feature: Authentication Unhappy Cases
  ...
  Scenario: Authentication with malformed JSON request
    ...
    And the response should have field "success" with boolean value "false"
  ...
  Scenario: Authentication with empty token
    Given WireMock is configured for "Google" OAuth provider
    When I send a POST request to "/auth/google" with body:
      """
      {
  ...
  Scenario: Authentication with invalid Google token
  ...
  Scenario: Authentication with expired Google token
  ...
  Scenario: Access protected endpoint with malformed Authorization header
  ...
  Scenario: Request with unsupported media type
    Given WireMock is configured for "Google" OAuth provider
  ...
  Scenario: Request with payload too large
    Given WireMock is configured for "Google" OAuth provider
  ...
  Scenario: Suspicious user agent detection
    Given WireMock is configured for "Google" OAuth provider
  ...
  Scenario: Suspicious header value is rejected
  ...
  Scenario: Missing X-Client-Platform header
    When I send a GET request to "/api/auth/me" with headers:
  ...
  Scenario: Missing X-Client-Version header
    When I send a GET request to "/api/auth/me" with headers:
      | Authorization        | Bearer gateway-access-token |
  ...
  Scenario: Missing X-Device-ID header
    When I send a GET request to "/api/auth/me" with headers:
      | Authorization        | Bearer gateway-access-token |
      | X-Client-Platform    | ios                         |
      | X-Client-Version     | 1.0.0                       |
  ...
  Scenario: Invalid X-Client-Platform header value
    When I send a GET request to "/api/auth/me" with headers:
      | Authorization        | Bearer gateway-access-token |
  ...
  Scenario: Invalid X-Client-Version header value
    When I send a GET request to "/api/auth/me" with headers:
      | Authorization        | Bearer gateway-access-token |
      | X-Client-Platform    | ios                         |
