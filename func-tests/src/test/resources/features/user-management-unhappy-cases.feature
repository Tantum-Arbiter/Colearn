Feature: User Management Unhappy Cases
  ...
  Scenario: Get user profile when user not found
  ...
  Scenario: Update user profile when Firebase is unavailable
  ...
  Scenario: Create child profile with invalid age
    ...
    And the response should have field "details.field" with value "birthDate"

  Scenario: Create child profile when limit exceeded
  ...
  Scenario: Update non-existent child profile
  ...
  Scenario: Delete child profile that doesn't exist
