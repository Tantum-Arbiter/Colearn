...
export function SettingsTipsOverlay({ isActive = true }: SettingsTipsOverlayProps) {
  const { hasSeenSettings, markSettingsViewed, shouldShowTutorial } = useTutorial();
  ...
  useEffect(() => {
    // Only trigger once and only when the screen is active
  ...
  }, [hasSeenSettings, shouldShowTutorial, opacity, scale, isActive, hasTriggered]);
        ...
        <Animated.View style={[styles.card, animatedCardStyle]}>
          <View style={styles.iconContainer}>
            <Text style={styles.icon}>
              ...
              {currentTip.id === 'notifications' && '🔔'}
              {currentTip.id === 'schedule' && '📅'}
              {currentTip.id === 'accessibility' && '👁️'}
            </Text>
          </View>
